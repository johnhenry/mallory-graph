...
/**
 * Converts a mallory-ts Graph3DUtils Mesh (a colour + a flat list of
 * triangular Faces, each three independent Vec3 corners) into a Three.js
 * BufferGeometry. Deliberately non-indexed: mallory-ts's faces don't share a
...
export function meshToGeometry(mesh: Mesh): THREE.BufferGeometry {
  ...
  for (const face of mesh.faces) {
    for (const vertex of face) {
      // mallory-ts's z is the function's height (z=f(x,y)); Three.js/
      // OrbitControls conventionally treat Y as "up", so height maps to
      // Three's y-axis and mallory-ts's y maps to Three's z-axis.
