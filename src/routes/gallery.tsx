...
import { encodeMultiGraphState } from "../lib/multi-graph-state.ts";
...
/**
 * Lists every graph saved from GraphCanvasMulti's "Save to gallery" button.
 * Opening one just navigates to /multi with the saved state encoded into
 * the hash -- reusing the exact hydrate-from-hash mechanism the URL-sync
 * feature already provides, so no separate "load by id" hydration path was
 * needed. Editing a reopened graph and hitting "Fork this view"/re-saving
...
function GalleryPage() {
  ...
  async function open(id: string) {
    try {
      const state = await getSavedGraphFn({ data: { id } });
      // A full navigation (not client-side routing) so /multi's mount-time
      // hash read (see GraphCanvasMulti's useMultiGraph) always runs fresh,
      // rather than depending on a hash-only change re-triggering it.
      window.location.href = `/multi#${encodeMultiGraphState(state)}`;
    } catch (e) {
        ...
        <ul>
          {entries.map((entry) => (
            <li key={entry.id} style={{ margin: "0.25rem 0" }}>
              <button type="button" onClick={() => open(entry.id)} style={{ font: "inherit" }}>
                {entry.title}
              </button>{" "}
